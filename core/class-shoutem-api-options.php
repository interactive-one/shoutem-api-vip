...
class ShoutemApiOptions {
	...
	private function get_checkbox_value( $key ) {
		if ( array_key_exists( $key,$_POST ) ) {
			$cb = sanitize_text_field( $_POST[ $key ] );
			if ( $cb == 'true' ) {
				return true;
			} else {
				return false;
	...
	private function update_options( &$options ) {
		...
		if ( ! empty( $_POST['brightcove_token'] ) ) {
			$options['brightcove_token'] = sanitize_text_field( $_POST['brightcove_token'] );
		}
			...
			<div class="wrap">
				...
				<form action="options-general.php?page=shoutem-api" method="post">
					...
					<table class="form-table">
						...
						<tr valign="top">
						<th scope="row"><?php esc_html_e( 'Brightcove API token' ) ?></th>
						<td><input type="text" name="brightcove_token" value="<?php echo esc_attr( $options['brightcove_token'] ); ?>" size="60" />
						</tr>
