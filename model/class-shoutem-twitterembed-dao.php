...
class ShoutemTwitterEmbedDao extends ShoutemDao {
	...
	function shortcode_twitterembed($atts, $content) {
		...
		$tweet_text = html_entity_decode($text_node->nodeValue);

		$timestamp = '';
		$timestamp_node = $xpath->query('a', $user_node)->item(0);
		if ($timestamp_node) {
			$timestamp = html_entity_decode($timestamp_node->textContent);
		}
		...
		$user_info_text = html_entity_decode($user_info_text);
		preg_match('/\s*(.*)\\((.*)\\)/i', $user_info_text, $user_info);

		return '<twitterdiv class="twitter-embed">'.
					'<twitterdiv class="twitter-icon"></twitterdiv>'.
  					'<twitterdiv class="name">'.esc_html(trim($user_info[1])).'</twitterdiv>'.
  					'<twitterdiv class="handle">'.esc_html($user_info[2]).'</twitterdiv>'.
  					'<twitterdiv class="tweet">'.esc_html($tweet_text).'</twitterdiv>'.
  					'<twitterdiv class="timestamp">'.esc_html($timestamp).'</twitterdiv>'.
				'</twitterdiv>';
	}
