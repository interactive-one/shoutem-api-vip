...
class ShoutemPostsDao extends ShoutemDao {
	...
	private function get_post( $post, $params ) {
		...
		$body = apply_filters( 'shoutem_shortcode_wrapper', $body, 'protected-iframe', 'shoutemprotectediframe' );
		...
		} elseif ( array_key_exists( 'commentable', $params ) ) {
...
function shoutem_html_load( $html ) {
	...
	$dom = new DOMDocument();
	libxml_use_internal_errors( true ); // VIP: Using @ is not enough to silence the warnings and errors that can happen here
