...
class ShoutemBrightcoveEmbedDao extends ShoutemDao {
	...
	function shortcode_wpcomwidgetembed($atts, $content) {
        ...
        if (function_exists("wpcom_vip_file_get_contents")) {
        	@$wp_com_widget = wpcom_vip_file_get_contents($url, false, $context);
        }
        else {
	...
	private function parse_object($brightcove_node, $xpath) {
		...
		$out = '<object>'.'<embed src="'.esc_url($url).'"';
		if ($paramWidth && $paramHeight) {
			$out .= ' width="'.esc_attr($paramWidth->getAttribute('value')).'"';
			$out .= ' height="'.esc_attr($paramHeight->getAttribute('value')).'"';
		}
		$out .= '></embed></object>';
		return $out;
	}
